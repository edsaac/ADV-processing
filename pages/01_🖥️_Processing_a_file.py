...
with col1:
    with st.expander(
        "Upload a netCDF4 file from the Vectrino Profiler 👇",
...
with col2:
    if st.button("I don't have a file"):
        with open("./assets/dummyADV.nc", "rb") as f:
            st.download_button(
                "Click here to download a dummy file", f.read(), "dummyADV.nc"
            )

"****"

if uploadedFile and ("upfile" in st.session_state.keys()):
    step_int = stx.stepper_bar(
        steps=["🏜️ Bottom Distance", "⏱️ Velocity Readings", "☀️ Summary"]
    )
    ...
    with tempfile.NamedTemporaryFile() as ncFile:
        ...
        if step_int == 0:
            ...
            with col1:
                st.image(
                    "assets/ADV_Distances.png", caption="Distances in the ADV setup"
                )

            with col2:
                with st.expander(
                    "😵‍💫  The entire dataset from the ADV", expanded=True
                ):
                    st.dataframe(
                        bott.style.format(
                            precision=4, subset=["time", "BottomDistance"]
                        ),
                        height=300,
                    )

                    st.download_button(
                        "😵‍💫  Click here to download the entire dataset as CSV",
            ...
            with col1:
                ...
                flumeDepth = st.number_input(
                    "Enter Flume depth [m]:", 0.00, 0.50, 0.15, 0.01, "%.2f"
                )
                probeDist = st.number_input(
                    "Enter SampleDistance [m]:", 0.00, 0.10, 0.05, 0.001, "%.3f"
                )
                filtered_bottom["SamplePositionZ"] = (
                    filtered_bottom["BottomDistance"] - probeDist
                )
                ...
                with st.expander("🍋 Our processed data", expanded=False):
                    ...
                    st.download_button(
                        "🍋 Click here to download this processed data as CSV",
            ...
            with col2:
                ...
                fig.update_layout(
                    ...
                    xaxis={"title": "Time (s)", "exponentformat": "power"},
                    yaxis={
                ...
                fig.add_hline(
                    ...
                    line={"width": 2, "color": "#1E90FF", "dash": "dash"},
                )
        ...
        #####################################
            ...
            ****
            ...
            with st.expander(
                "😵‍💫  The entire velocity readings from the ADV", expanded=True
            ):
                st.dataframe(vels.style.format(precision=4), height=200)
        ...
        #####################################
            ...
            with col2:
                ...
                fig.update_layout(
                    ...
                    xaxis={"title": "Velocity (m/s)", "exponentformat": "power"},
                    yaxis={
                        "title": "Elevation (m)",
                        "exponentformat": "power",
                        "range": [0, 1.1 * d],
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        name="Sampling position",
                        mode="markers",
                        marker={"size": 10, "symbol": "circle-dot"},
                    )
                )
